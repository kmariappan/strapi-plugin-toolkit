...
export class StrapiTypeBuilder {
  constructor(private keys: StrapiKey[], private contents: StrapiContent[]) {}
  ...
  public generate = () => {
    ...
    this.keys.forEach((key) => {
      if (key.name === 'ContentKeys') {
        key.data.forEach((k) => {
          const content = this.contents[0].data[k];
          try {
    ...
    switch (field.type) {
      ...
      case 'relation': {
        ...
        if (field.target) {
          ...
          prefix = this.contents[0].data[k]?.globalId;
        }
